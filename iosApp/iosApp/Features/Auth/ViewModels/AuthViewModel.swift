...
class AuthViewModel: ObservableObject {
        ...
        if let useCase = signInUseCase {
            Task {
                do {
                    ...
                    let _ = try await useCase.invoke(email: email, password: password)
        ...
        if let useCase = signUpUseCase {
            Task {
                do {
                    ...
                    let _ = try await useCase.invoke(email: email, password: password, username: username)
        ...
        if let useCase = sendPasswordResetUseCase {
            Task {
                do {
                    ...
                    let _ = try await useCase.invoke(email: email)
    ...
    func authenticateWithBiometrics(navigator: AppNavigator) {
