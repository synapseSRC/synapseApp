...
def main():
    ...
    artifact_path = os.environ.get('ARTIFACT_PATH') # New variable for the file path
    ...
    try:
        # Check if we have a valid artifact to upload
        if artifact_path and os.path.exists(artifact_path):
            logger.info(f"Artifact found. Uploading: {artifact_path}")
            url = f"https://api.telegram.org/bot{bot_token}/sendDocument"
            
            # For documents, the text is passed as a 'caption'
            data["caption"] = final_message
            
            with open(artifact_path, 'rb') as document_file:
                files = {'document': document_file}
                # Use data= and files= (multipart/form-data) instead of json=
                # Increased timeout to 300 seconds to allow time for the upload
                response = requests.post(url, data=data, files=files, timeout=300)
                response.raise_for_status()
                logger.info("Notification and artifact uploaded successfully.")
                
        else:
            logger.info("No artifact provided or file not found. Sending text message only.")
            url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
            
            # For standard messages, it is passed as 'text'
        ...
        logger.error(f"Failed to send message/artifact: {e}")
