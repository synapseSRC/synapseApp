...
val linkPreviewListAdapter = object : ColumnAdapter<List<LinkPreview>, String> {
    override fun decode(databaseValue: String): List<LinkPreview> {
        return if (databaseValue.isBlank()) emptyList() else try {
            Json.decodeFromString<List<LinkPreview>>(databaseValue)
        } catch (e: Exception) {
            emptyList()
        }
    }

    override fun encode(value: List<LinkPreview>): String {
